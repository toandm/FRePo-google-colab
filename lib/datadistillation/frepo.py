...
def init_proto(
    ds, num_prototypes_per_class, num_classes, class_subset=None, seed=0, scale_y=False
):
    ...
    ds = ds.group_by_window(
        key_func=lambda x, y: y, reduce_func=reduce_func, window_size=window_size
    )
...
def create_proto_state(rng, model, learning_rate_fn, optimizer="lamb"):
    ...
    variables = flax_core.unfreeze(model.init({"params": rng}))

    state = ProtoState.create(
...
def nn_feat_fn(x, state, has_bn=False, use_ema=False):
    if use_ema:
        if has_bn:
            variables = {
                "params": state.ema_average,
                "batch_stats": state.ema_average_batch,
            }
        else:
...
def proto_train_step(state, nn_state, batch, use_flip=False, feat_fn=None):
    ...
    grad_x = jnp.concatenate(
        [v.reshape(1, -1) for k, v in grads.items() if "x_proto" in k], axis=0
    )
    grad_y = jnp.concatenate(
        [v.reshape(1, -1) for k, v in grads.items() if "y_proto" in k], axis=0
    )
...
def compute_feat_kpp(x_proto, nn_state, feat_fn=None, reg=1e-6, chunk_size=500):
    ...
    if train_size <= chunk_size:
        ...
        for i in range(train_size // chunk_size):
            x_proto_list.append(
                feat_fn(x_proto[i * chunk_size : (i + 1) * chunk_size], nn_state)
            )

        if train_size % chunk_size != 0:
            x_proto_list.append(
                feat_fn(x_proto[train_size // chunk_size * chunk_size :], nn_state)
            )
...
def jit_fn(
    nn_train_step, nn_eval_step, feat_fn, feat_fn_eval, use_flip=False, use_pmap=False
):
    ln_train_step = functools.partial(
        proto_train_step, feat_fn=feat_fn, use_flip=use_flip
    )
    ln_eval_step = functools.partial(proto_eval_step, feat_fn=feat_fn_eval)

    if use_pmap:
        jit_train_step = jax.pmap(
            ln_train_step, axis_name="batch", in_axes=(0, 0, 0)
        )  # state, nn_state, batch
        jit_eval_step = jax.pmap(
            ln_eval_step, axis_name="batch", in_axes=(0, None, 0, 0, None)
        )  # x_proto, y_proto, k_pp_reg, nn_state, batch
        jit_nn_train_step = jax.pmap(
            nn_train_step, axis_name="batch", in_axes=(0, None, None)
        )  # state, batch, rng
        jit_nn_eval_step = jax.pmap(
            nn_eval_step, axis_name="batch", in_axes=(0, None)
        )  # state, batch
        jit_nn_train_step_online = jax.pmap(
            nn_train_step, axis_name="batch", in_axes=(0, None, None)
        )  # state, batch, rng
    else:
    ...
    return (
        ...
        jit_nn_train_step,
        jit_nn_eval_step,
        jit_nn_train_step_online,
    )


def train_on_proto(
    ds, nn_state, jit_nn_train_step, diff_aug, rng, num_updates=1000, has_bn=False
):
    ...
    for step, (img, lb) in tqdm.tqdm(
        zip(range(num_updates), ds.as_numpy_iterator()), desc="Train on distilled data"
    ):
        rng, train_rng = jax.random.split(rng)
        img = diff_aug(train_rng, img)
        nn_state, _ = jit_nn_train_step(
            nn_state, {"image": img, "label": lb}, train_rng
        )
    ...
    nn_state = dataclasses.replace(
        nn_state, **{"ema_hidden": EMAState.hidden, "ema_average": EMAState.average}
    )
    if has_bn:
        EMABatch(nn_state.batch_stats)
        nn_state = dataclasses.replace(
            nn_state,
            **{
                "ema_hidden_batch": EMABatch.hidden,
...
def eval_on_proto_nn(ds_test, nn_state, jit_nn_eval_step, writer=None, step=0, name=""):
    ...
    for batch in tqdm.tqdm(
        ds_test.as_numpy_iterator(), desc="Evaluate using nn predictor"
    ):
    ...
    if writer is not None:
        writer.write_scalars(
            step + 1, {f"{name}/nn_{k}_mean": v for k, v in nn_summary_mean.items()}
        )
        writer.write_scalars(
            step + 1, {f"{name}/nn_{k}_std": v for k, v in nn_summary_std.items()}
        )
        writer.flush()
    else:
        logging.info(
            str({f"{name}/nn_{k}_mean": v for k, v in nn_summary_mean.items()})
        )

    return nn_summary_mean


def eval_on_proto_krr(
    ds_test,
    x_proto,
    y_proto,
    nn_state,
    jit_eval_step,
    jit_compute_feat_kpp,
    use_flip=False,
    writer=None,
    step=0,
    name="",
    ...
    krr_eval_metrics = []
    for batch in tqdm.tqdm(
        ds_test.as_numpy_iterator(), desc="Evaluate using krr predictor"
    ):
        img, lb = process_batch(batch, use_pmap=False)
        metrics = jit_eval_step(
            x_proto, y_proto, k_pp_reg, nn_state, {"image": img, "label": lb}
        )
    ...
    if writer is not None:
        writer.write_scalars(
            step + 1, {f"{name}/ln_{k}_mean": v for k, v in krr_summary_mean.items()}
        )
        writer.write_scalars(
            step + 1, {f"{name}/ln_{k}_std": v for k, v in krr_summary_std.items()}
        )
        writer.flush()
    else:
        logging.info(
            str({f"{name}/nn_{k}_mean": v for k, v in krr_summary_mean.items()})
        )
...
def proto_eval(
    ...
    ds_proto = (
        ...
        .batch(batch_size=min(y_proto.shape[0], 500))
    )
...
def get_sample_proto(state, rng, bs=500, use_pmap=False):
    ...
    if y_proto.shape[0] < bs:
        ...
        sample_idx = jax.random.choice(
            rng, y_proto.shape[0], shape=(bs,), replace=False
        )
        img, lb = x_proto[sample_idx], y_proto[sample_idx]
    return img, lb


def proto_train_and_evaluate(
    config,
    ph,
    create_online_state,
    create_eval_state,
    ...
    use_flip=False,
    num_online_eval_updates=100,
    diff_aug=None,
    max_online_updates=50,
    ...
    jit_get_sample_proto = jax.jit(
        functools.partial(
            get_sample_proto, use_pmap=use_pmap, bs=config.eval_batch_size
        )
    )
    ...
    learning_rate_fn = optax.cosine_decay_schedule(
        init_value=config.learning_rate, decay_steps=num_train_steps, alpha=0.1
    )
    ...
    for i in range(num_nn_state):
        ...
        if use_pmap:
            nn_state = jit_vmap_create_online_state(
                jax.random.split(init_rng, jax.device_count())
            )
            nn_state = dataclasses.replace(
                nn_state, **{"step": flax.jax_utils.replicate(init_step)}
            )
        else:
    ...
    nn_train_step = functools.partial(
        train_step, has_feat=True, has_bn=has_bn, loss_type=mean_squared_loss
    )
    nn_eval_step = functools.partial(
        eval_step,
        has_feat=True,
        has_bn=has_bn,
        use_ema=True,
    ...
    get_jit_fn = functools.partial(
        jit_fn,
        feat_fn=feat_fn,
        feat_fn_eval=feat_fn_eval,
        use_flip=use_flip,
        use_pmap=use_pmap,
    )
    (
        ...
        jit_nn_eval_step,
        jit_nn_train_step_online,
    ) = get_jit_fn(nn_train_step, nn_eval_step)

    nn_train_step_eval = functools.partial(
        train_step, has_feat=True, has_bn=False, loss_type=mean_squared_loss
    )
    nn_eval_step_eval = functools.partial(
        eval_step, has_feat=True, has_bn=False, loss_type=mean_squared_loss
    )
    (
        jit_train_step_eval,
        jit_eval_step_eval,
        jit_nn_train_step_eval,
        jit_nn_eval_step_eval,
        _,
    ) = get_jit_fn(nn_train_step_eval, nn_eval_step_eval)
    ...
    evaluate_proto = functools.partial(
        proto_eval,
        ds_test=ds_test,
        num_online_eval_updates=num_online_eval_updates,
        jit_nn_train_step=jit_nn_train_step_eval,
        diff_aug=diff_aug,
    ...
    for step, batch in zip(
        range(step_offset, num_train_steps), ds_train.as_numpy_iterator()
    ):
        # --------------------------------------
        # Train image
        # --------------------------------------
        ...
        state, metrics = jit_train_step(
            state, nn_state, {"image": feat, "label": label}
        )
        ...
        if step == step_offset:
            logging.info(
                "Initial compilation completed. Elapsed: {}s".format(
        ...
        if (step + 1) in log_steps or (step + 1) % steps_per_log == 0:
            ...
            summary["monitor/steps_per_second"] = steps_per_log / (
                time.time() - train_metrics_last_t
            )
            summary["monitor/learning_rate"] = learning_rate_fn(step)

            x_proto, y_proto = jit_get_proto(state)
            x_proto_norm = jnp.linalg.norm(
                x_proto.reshape(x_proto.shape[0], -1), ord=2, axis=-1
            ).mean()
        ...
        if nn_step > max_online_updates:
            ...
            nn_state, _ = jit_nn_train_step_online(
                nn_state, {"image": img, "label": lb}, train_rng
            )
        ...
        if (step + 1) % steps_per_checkpoint == 0 or step + 1 == num_train_steps:
            ...
            if accuracy > best_val_acc:
                ...
                if use_pmap:
                    ...
                    state = dataclasses.replace(
                        state,
                        **{"step": step, "epoch": epoch, "best_val_acc": best_val_acc},
                    )

                try:
                    save_checkpoint(
                        state,
                        os.path.join(workdir, "best_ckpt"),
        ...
        if (step + 1) in log_steps or (step + 1) % save_ckpt == 0:
            try:
                save_checkpoint(
                    state, os.path.join(workdir, "saved_ckpt"), step + 1, keep=15
                )
            except Exception as e:
                print(e)
            finally:
        ...
        if image_saver:
            if (
                (step + 1) in log_steps
                or (step + 1) % steps_per_save_image == 0
...
def proto_evaluate(
    ...
    nn_train_step = functools.partial(
        train_step, has_feat=True, has_bn=has_bn, loss_type=loss_type
    )
    nn_eval_step = functools.partial(
        eval_step, has_feat=True, has_bn=has_bn, loss_type=loss_type
    )
    ...
    if use_pmap:
        jit_compute_feat_kpp = jax.pmap(
            functools.partial(compute_feat_kpp, feat_fn=feat_fn_eval), in_axes=(None, 0)
        )  # x_proto, nn_state
    else:
        jit_compute_feat_kpp = jax.jit(
            functools.partial(compute_feat_kpp, feat_fn=feat_fn_eval)
        )
    ...
    if use_pmap:
        ...
        for step, (img, lb) in tqdm.tqdm(
            zip(range(num_online_eval_updates), ds_train.as_numpy_iterator()),
            desc="Train on distilled data",
        ):
            rng, train_rng = jax.random.split(rng)
            img = diff_aug(train_rng, img)
            nn_state, metrics = jit_nn_train_step(
                nn_state, {"image": img, "label": lb}, train_rng
            )
            train_metrics.append(metrics)
            ...
            if (step + 1) % steps_per_log == 0:
                ...
                if writer is None:
                    ...
                    if name != "":
                        writer.write_scalars(
                            step + 1,
                            {
                                f"train/{name}/nn_{k}_mean": v
                                for k, v in nn_summary_mean.items()
                            },
                        )
                        writer.write_scalars(
                            step + 1,
                            {
                                f"train/{name}/nn_{k}_std": v
                                for k, v in nn_summary_std.items()
                            },
                        )
                    else:
                        writer.write_scalars(
                            step + 1,
                            {
                                f"train/nn_{k}_mean": v
                                for k, v in nn_summary_mean.items()
                            },
                        )
                        writer.write_scalars(
                            step + 1,
                            {f"train/nn_{k}_std": v for k, v in nn_summary_std.items()},
                        )
                    writer.flush()
                train_metrics = []

                nn_acc_mean = eval_on_proto_nn(
                    ds_test,
                    nn_state,
                    jit_nn_eval_step,
                    writer,
                ...
                if eval_krr:
                    krr_acc_mean = eval_on_proto_krr(
                        ds_test,
                        x_proto,
                        y_proto,
                        nn_state,
        ...
        logging.info(
            f"Evaluate on {num_eval} models, mean: {np.mean(acc_list)}, std: {np.std(acc_list)}"
        )
