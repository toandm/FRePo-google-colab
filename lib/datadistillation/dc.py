...
class DCMethod(BaseDistillationMethod):
    ...
    def compute_gradient_real(
        ...
        def forward_loss(params):
            ...
            # Use mutable to allow batch_stats updates during forward pass
            if 'batch_stats' in variables:
                out, _ = nn_state.apply_fn(variables, batch['image'], train=True, mutable=['batch_stats'])
            else:
                out = nn_state.apply_fn(variables, batch['image'], train=True)

            # Handle tuple output (e.g., when output='feat_fc')
    ...
    def compute_gradient_synthetic(
        ...
        def forward_loss(params):
            ...
            # Use mutable to allow batch_stats updates during forward pass
            if 'batch_stats' in variables:
                out, _ = nn_state.apply_fn(variables, x_syn, train=True, mutable=['batch_stats'])
            else:
                out = nn_state.apply_fn(variables, x_syn, train=True)
